        ...
        if os.path.isdir(label_path):  # If it is a folder
            for image_file in os.listdir(
                ...
                if image_file.endswith(".png"):  # If the file extension is .png
                    ...
                    img = img.resize(
                        (28, 28), Image.ANTIALIAS
                    )  # Resize with a higher quality downsampling filter
...
def process_and_save_data(training_dir, validation_dir, testing_dir, output_file):
    ...
    """
    training_images, training_labels = load_images_from_folder(
        training_dir
    )  # Load training data
    validation_images, validation_labels = load_images_from_folder(
        validation_dir
